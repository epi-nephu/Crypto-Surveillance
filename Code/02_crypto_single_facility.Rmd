---
title: 'Cryptosporidiosis - Aquanation Ringwood'
author_details: "Alana Little, NEPHU (alana.little@austin.org.au)"
version_details: "v1.0, 17/09/2025"
output: 
  html_document: 
    code_folding: none
    toc: true
    ...
    toc_float: true
knit: (function(input, ...) {
  rmarkdown::render(input, output_file = paste0("Crypto - ", "Aquanation Ringwood - ", Sys.Date()), output_dir = "../Reports/Single Facilities") })
---
...
```{r}
# Facility
facility_raw  <- "Aquanation Ringwood (Aquatic facility)"
facility_name <- "Aquanation Ringwood"

# Dates
extract_date <- lubridate::ymd(Sys.Date())
file_date    <- format(extract_date, format = "%Y%m%d")

# 1-month lookback period for facility-level analyses
lookback_start <- lubridate::ymd(extract_date - months(1))
