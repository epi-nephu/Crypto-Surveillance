...
output: 
  html_document: 
    ...
    toc_float: true
knit: (function(input, ...) {
  rmarkdown::render(input, output_file = paste0(Sys.Date(), "TEST Crypto in Victoria"), output_dir = "../Reports") })
---
...
linelist_exposures <- linelist_cases %>% 
  ...
  dplyr::filter(!is.na(name),
                name != ",") %>%
  #
  # Are you getting a tidyr::separate_longer_delim() error? If so, uncomment lines 177-181
  ...
  # exposure suburb fields. You can then comment out lines 177-181, re-extract the PHESS 
...
# dplyr::filter(n_category != n_name | n_name != n_suburb | n_suburb != n_category)
  ...
  dplyr::mutate(
    facility_clean = dplyr::case_when(
      ...
      facility_clean == "Big4 Yarra Valley Park Lane Holiday Park" ~ "BIG4 Yarra Valley Park Lane",
      facility_clean == "Bluefit Swimming - Mill Park" ~ "BlueFit Swimming",
      facility_clean == "Brunswickcity Baths" ~ "Brunswick City Baths",
      facility_clean == "Casey Race Ccc" ~ "Casey Recreation & Aquatic Centre",
      facility_clean == "Discovery Parks - Echuca" ~ "Discovery Park - Echuca",
      facility_clean == "Discovery Park Maidens Inn" ~ "Discovery Park - Maidens Inn",
      facility_clean == "Dreamworld - Gold Coast Theme Park" ~ "Dreamworld Gold Coast",
      facility_clean == "Keilor East Leisure Centre" ~ "East Keilor Leisure Centre",
      facility_clean == "Knox Leisureworks Ymca" ~ "Knox Leisureworks YMCA",
      facility_clean == "Hepburn Springs Bathouse" ~ "Hepburn Springs Bathhouse",
      facility_clean == "John Sugden Swim School Kilsyth South" ~ "John William Sugden Swim School - Kilsyth South",
      ...
      facility_clean == "Jump Swim School Flemington" ~ "Jump! Swim Schools Flemington",
      facility_clean == "Jump! Swim School Lilydale" ~ "Jump! Swim Schools Lilydale",
      facility_clean == "Kic Swim School" ~ "KIC Swim School",
      facility_clean == "Kiddieswim Australia Pty Ltd" ~ "Kiddieswim Australia",
      facility_clean == "Kings Swim Chirnside Park" ~ "Kingswim Chirnside Park",
      facility_clean == "Nrma Bairnsdale Riverside Holiday Park" ~ "NRMA Bairnsdale Riverside Holiday Park",
      facility_clean == "Nrma Eastern Beach Holiday Park" ~ "NRMA Eastern Beach Holiday Park",
      facility_clean == "Nrma Echuca Holiday Park" ~ "NRMA Echuca Holiday Park",
      facility_clean == "Paul Saddler Swimland" ~ "Paul Sadler Swimland Essendon",
      facility_clean == "Racv Cobram Resort" ~ "RACV Cobram Resort",
      facility_clean == "Racv Noosa Resort" ~ "RACV Noosa Resort",
      facility_clean == "Royal Melbourne" ~ "Royal Melbourne Hospital",
      facility_clean == "Sense Of Self" ~ "Sense of Self Day Spa",
      facility_clean == "Urbnsurf Melbourne" ~ "URBNSURF Melbourne",
      facility_clean == "Watermarc Banyule" ~ "WaterMarc Banyule",
      facility_clean == "Wet'n'wild Gold Coast" ~ "Wet'n'Wild Gold Coast",
      ...
      facility_clean == "Wodonga Abattoir 54 Kelly St Wodonga Vic 3690" ~ "Wodonga Abattoir",

      # Remove private residences
      facility_clean == "1333 Murray River Rd Talgarno" ~ NA_character_,
      facility_clean == "283 Spring St Melbourne" ~ NA_character_,
      facility_clean == "55 Margaret Court Dr" ~ NA_character_,
      facility_clean == "7 Sinnett Court West Wodonga Vic 3690" ~ NA_character_,
      facility_clean == "95/33 La Trobe St Melbourne" ~ NA_character_,
      facility_clean == "Body Corporate Ps 428541d" ~ NA_character_,
      facility_clean == "Jayden And Tayla's Residence" ~ NA_character_,
      facility_clean == "Taran Gisbornes Residence" ~ NA_character_,
      facility_clean == "Private Residence - 15 Gerald St Ferntree Gully" ~ NA_character_,
      facility_clean == "Private Residence - 21 Henbury Street Huntly" ~ NA_character_,
      ...
      facility_clean == "Bangkok Thailand" ~ NA_character_,
      facility_clean == "Benalla Rural City Council" ~ NA_character_,
      facility_clean == "Broken River Benalla" ~ NA_character_,
      facility_clean == "Fleming Park Brunswick" ~ NA_character_,
      facility_clean == "Lake Eildon" ~ NA_character_,
      facility_clean == "Mansfield Lake - Lake Eildon" ~ NA_character_,
      facility_clean == "Pambula Beach" ~ NA_character_,
      facility_clean == "Suvarnabhumi Airport Bangkok Thailand" ~ NA_character_,
      facility_clean == "Suvarnabhumi Airport" ~ NA_character_,
...
**Victorian public facilities with two or more cryptosporidiosis notifications, `r format(extract_date - 42, format = "%d %B %Y")` to `r format(extract_date, format = "%d %B %Y")`**
...
recent_cases <- facility_cases %>% 
  ...
  #
  dplyr::filter(symptom_onset >= extract_date - 42) %>% 
  #
...
`r if(nrow(recent_cases) == 0) paste0("There are no public facilities in Victoria with two or more cryptosporidiosis cases within the past 42 days.")`

`r if(nrow(recent_cases) > 0) table_recent_cases`

`r if(nrow(recent_cases) > 0) paste0("During the past 42 days there have been ", nrow(recent_cases), " public facilities in Victoria with two or more cryptosporidiosis cases within the past 42 days. Please review the linelist, epicurve, and swimlane plot in the relevant LPHU section of the report to assess whether these facilities represent an outbreak that requires escalation.")`
